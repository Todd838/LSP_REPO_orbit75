...
Package Structure: org.howard.edu.lsp.assignment2
Program Flow: follows the classic ETL pattern:
Extract: read products.csv using BufferedReader. Skip header and blank lines.
Transform:
...
Load: write transformed data to data/transformed_products.csv with header.
Separation of Concerns: Each ETL step is in its own method: extract(), transform(), load().
